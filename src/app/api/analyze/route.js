  ...
  try {
    ...
    if (mode === 'profile' && profile) {
      ...
      const systemPrompt = 'אתה אתי אטל - יועצת בריאות ותזונה התנהגותית בגישת NLP.\n'
        ...
        + 'חובה: עברית תקנית בלבד. אל תמציאי פרטים. אל תכתבי כותרת ראשית בתחילת התשובה — התחילי ישר עם הסעיף הראשון. כתבי HDL ולא BDL.\n'
        + 'כללי מבנה נוקשים לכרטיסיות:\n'
        + '1. כל סעיף מוגבל ל-3 עד 4 משפטים בלבד (למעט סעיף הבדיקות שיכול להגיע ל-5 משפטים). הציגי מסר ממוקד וחד.\n'
        + '2. אל תחזרי על ערכי מעבדה תקינים מהטבלה. התמקדי בפרשנות ובחיבור לסטרס, שינה ותזמון אכילה.\n'
        + '3. כל משפט חייב להיות שלם, קולח וסגור לחלוטין. אל תפסיקי משפט באמצע.\n'
        + '4. אם קיימים ערכים חריגים נוספים בנתונים (כמו IgG, גמא גלובולין, פוטסיום או FLC) - חובה לשלב את המשמעות הפיזיולוגית והאנטי-דלקתית שלהם בתוך סעיף הבדיקות.\n\n'
      ...
      const [msg1, msg2] = await Promise.all([
        ...
        client.messages.create({
          model: 'claude-sonnet-4-6',
          max_tokens: 1000,
          messages: [{ role: 'user', content: systemPrompt + baseData + '\nחובה לפתוח כל סעיף בכותרת המדויקת שלו, ולהפריד בין הסעיפים באמצעות השורה המפרידה -- בלבד (ללא מפרידים נוספים). כתבי את 3 הסעיפים הבאים:\n\n**🩸 מה אומרות הבדיקות**\nהתייחסי לערכים חריגים מובנים, ובפרט לחריגות הנוספות שהוזנו ידנית בטקסט (כמו IgG, פוטסיום, גמא גלובולין, FLC וכו\'). הסבירי את משמעות העומס הדלקתי והסטרס הפיזיולוגי במילים פשוטות והנחי לפעולה או לבירור רפואי.\n\n--\n\n**🥗 המלצות תזונה ותוספים**\n[תוכן הסעיף]\n\n--\n\n**🎯 3 צעדים למחר**\n(ממוספרים, קצרים וריאליסטיים' + stepsNote + ')' }]
        })
      ])
