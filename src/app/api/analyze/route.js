...
export async function POST(request) {
  try {
    ...
    if (mode === 'agentInstructions' && body.clientPassword) {
      ...
      const prompt = `אתה מסייע ליועצת תזונה לסכם פרופיל לקוחה לצורך Agent AI.
...
`
      const prompt = 'את אתי אטל — יועצת תזונה התנהגותית. כתבי משוב ל-' + name + '.\n' +
        genderNote + '\n' + voiceGuide + '\n' +
        'כותרת: ' + reportTitle + '\n' +
    ...
    if (logs && !mode) {
      ...
      const msg = await client.messages.create({
        ...
        max_tokens: 700,
        messages: [{ role: 'user', content:
          'את אתי אטל — יועצת תזונה התנהגותית. כתבי משוב קצר ל-' + name + '.\n'
          + genderNote + '\n'
