...
הפק ניתוח מקיף לאתי לקראת הפגישה. **הצף הכל — אל תסנן ואל תבחר. כל דפוס, כל אמונה, כל הזדמנות.** כתוב בעברית ישירה ומקצועית.

---

## ⏱️ מבנה פגישה מומלץ — 60 דקות
בהתאם למה שעלה בשאלון הספציפי הזה — הצע חלוקת זמן ריאלית. ציין לכל קטע: מה לעשות, כמה זמן, ולמה זה עכשיו. אם יש חומר כבד (אמונות עמוקות, אקולוגיה מורכבת) — תן לזה יותר זמן.

---

## 1. מטרה מוגדרת כהלכה (Well-Formed Outcome)
נסח את המטרה מחדש: חיובי, ספציפי, בשליטתה, מחובר לתחושה גופנית. הצג גם את ה"הוכחה" שתקבע שהגיעה.
**💡 בפגישה:** כיצד להציג את הניסוח — מה לומר, כיצד לבדוק שמרגיש נכון, ומה לעשות אם תדחה אותו.

---

## 2. כל האמונות המגבילות — אחת אחת
לכל אמונה שעולה מהשאלון:
**ציטוט מדויק** | **קטגוריה** (חוסר אונים / חוסר ערך / חוסר שייכות / שליטה חיצונית) | **המנגנון** — איך האמונה עובדת עליה בחיי היומיום | **שאלת ערעור** ממוקדת לניסוח שלה
**💡 בפגישה:** מה לומר, מה לא לומר, ואיזו טכניקה קונקרטית להשתמש.
אל תדלג על אמונה גם אם נראית שולית — לפעמים הדרגה קוברת את המרכזית.

---

## 3. דפוסי שפה — רשימה מלאה
סרוק את כל התשובות לדפוסים הבאים. לכל דפוס שמצאת:
**ציטוט** | **קטגוריה:** הכללה (תמיד/אף פעם/כולם) / חוק פנימי (חייבת/אסור/צריך) / סיבתיות (X גורם לY) / מחיקה (מה לא נאמר)
**💡 בפגישה:** שאלת הנגדה קצרה + מה לצפות כשתשאלי אותה.

---

## 4. אקולוגיה — כל ההתנגדויות הצפויות
מה היא עלולה להגן עליו — גם אם לא אמרה ישירות. כלול: מה יאבד אם תצליח, מי עלול להיפגע, מה ישתנה בחייה שאינה בטוחה שרוצה.
**💡 בפגישה:** לכל נקודת חיכוך — גישה ספציפית שלא תיצור התנגדות.

---

## 5. עוגני כוח — כל מה שמצאת
ציטוטים ישירים המראים חוזק, עמידות, משאב. לכל עוגן — פוטנציאל השימוש בפגישה.
**💡 בפגישה:** איך לגייס את הכוח הזה — שאלה שתגרום לה להרגיש אותו בגוף.

---

## 6. הרגעים הפגיעים + מבנה החיסון
זהה את כל הרגעים שעלו (לא רק הכי קשה — גם הפחות קשים). לכל רגע — בנה חיסון מותאם: פעולה קונקרטית + עוגן + משפט פנימי.
**💡 בפגישה:** כיצד לתרגל את החיסון יחד בשיחה — שיצא מהפה שלה, לא מפיך.

---

## 7. מה עוד חשוב לשאול
מה לא עלה בשאלון אבל צריך לברר בפגישה. שאלות ספציפיות עם הסבר לכל אחת — למה חשוב לדעת את זה עכשיו.`
...
ללא מבוא. ללא סיכום. ללא הסברים בסוגריים. רק התוכן עצמו — חם, ממוקד, שלה.`
    ...
    if (mode === 'logsReport') {
      ...
      const prompt = 'את אתי אטל — יועצת תזונה התנהגותית. כתבי משוב ל-' + name + '.\n' +
        genderNote + '\n' +
        'כותרת: ' + reportTitle + '\n' +
        'חובה: שפה חמה וישירה, כמו מאמנת שמכירה אותה. מספרים ספציפיים מהיומן בלבד.\n' +
        (nlpSummary ? 'מצב רגשי היום: ' + nlpSummary + '\nחשוב: אם הלחץ או העייפות גבוהים — חברי בין המצב הרגשי לדפוסי האכילה שנרשמו. זו תזונה התנהגותית.\n' : '') +
        'נתונים:\n' + String(logsSummary).substring(0, 1500) + '\n\n' +
        '# ' + reportTitle + '\n\n' +
        '**✨ מה עבד**\nהצלחה אחת ספציפית עם מספרים.\n\n' +
        '**🎯 לשיפור**\nנקודה אחת — מה בפועל מול היעד.\n\n' +
        '**🚀 3 צעדים**\nספציפיים עם מספרים.\n\n' +
        '**💚 מסר**\nמשפט אחד מעצים.'
      const msg = await client.messages.create({
        model: 'claude-sonnet-4-6',
