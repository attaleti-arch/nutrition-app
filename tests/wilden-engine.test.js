...
test('רוחות במכונה: עם שואב נשאבות, בלי שואב חוטפות', () => {
  ...
  // אין מי שייפלט, ולכן זו שאיבה של מטבעות: הבסיס ועוד עשרה
  assert.equal(sucked.run.coinsTaken, WIND_COINS + SUCK_COINS)
...
// ── "תזוזה ימינה מזיזה מפה שמאלה" ──
// המפה מסובבת כדי שהמסלול יצביע לכיוון ההליכה, ומנוע המפה לא יודע על
// הסיבוב. זה החישוב שמתקן את זה — ובלעדיו, בהליכה דרומה (סיבוב 180°),
// כל גרירה הפוכה.
import { panDelta, pinchPoint, pinchZoom } from '../src/app/wilden/engine/geo.js'

test('מפה: גרירה באצבע נכונה בכל זווית סיבוב', () => {
  const near = (a, b, msg) => assert.ok(Math.abs(a[0] - b[0]) < 1e-6 && Math.abs(a[1] - b[1]) < 1e-6,
    `${msg}: [${a.map(n => n.toFixed(2))}] במקום [${b.map(n => n.toFixed(2))}]`)
  // בלי סיבוב: גרירה ימינה מזיזה את המבט שמאלה, כלומר המפה הולכת עם האצבע
  near(panDelta(10, 0, 0), [-10, 0], 'צפון למעלה, ימינה')
  near(panDelta(0, 10, 0), [0, -10], 'צפון למעלה, למטה')
  // הולכים דרומה: המפה מסובבת 180°, והתיקון הופך את שני הצירים
  near(panDelta(10, 0, 180), [10, 0], 'הליכה דרומה, ימינה')
  near(panDelta(0, 10, 180), [0, 10], 'הליכה דרומה, למטה')
  // תשעים מעלות: ימינה על המסך הוא למטה במפה
  near(panDelta(10, 0, 90), [0, -10], 'הליכה מזרחה, ימינה')
  // והאורך תמיד נשמר — גרירה לא "מתקצרת" בזווית
  for (const deg of [0, 37, 90, 133, 180, 271, 359]) {
    const [x, y] = panDelta(8, -5, deg)
    assert.ok(Math.abs(Math.hypot(x, y) - Math.hypot(8, 5)) < 1e-6, `אורך נשמר ב-${deg}°`)
  }
})

// ── "הזום בצביטה מזיז באופן מוזר את טבעת המסלול" ──
// אותו באג, בגרסת שתי אצבעות: הנקודה שבין האצבעות צריכה לעבור דרך
// הסיבוב לפני שמוסרים אותה למנוע המפה, אחרת הזום מתרחש סביב נקודה אחרת
// לגמרי והמסלול "בורח" הצידה.
test('מפה: צביטה — הנקודה שבין האצבעות נכונה גם כשהמפה מסובבת', () => {
  const near = (a, b, msg) => assert.ok(Math.abs(a[0] - b[0]) < 1e-6 && Math.abs(a[1] - b[1]) < 1e-6,
    `${msg}: [${a.map(n => n.toFixed(2))}] במקום [${b.map(n => n.toFixed(2))}]`)
  // אלמנט המפה גדול מהחלון: 600×400. מרכז החלון הוא מרכז האלמנט.
  near(pinchPoint(0, 0, 0, 600, 400), [300, 200], 'צביטה במרכז נשארת במרכז')
  near(pinchPoint(0, 0, 137, 600, 400), [300, 200], 'וגם בכל זווית')
  // 50 פיקסלים ימינה על המסך, כשהמפה מסובבת 90° — זה "למטה" במפה
  near(pinchPoint(50, 0, 90, 600, 400), [300, 250], 'ימינה במסך = למטה במפה')
  near(pinchPoint(50, 0, 180, 600, 400), [250, 200], 'בהליכה דרומה — הצד השני')
  // והמרחק מהמרכז נשמר, אחרת הזום היה זוחל
  for (const deg of [0, 45, 90, 200, 330]) {
    const [x, y] = pinchPoint(30, -20, deg, 600, 400)
    assert.ok(Math.abs(Math.hypot(x - 300, y - 200) - Math.hypot(30, 20)) < 1e-6, `מרחק נשמר ב-${deg}°`)
  }
  // וכמה רמות זום: אצבעות שהתרחקו פי שתיים = רמה אחת פנימה
  assert.ok(Math.abs(pinchZoom(100, 200) - 1) < 1e-9, 'פי שתיים = +1')
  assert.ok(Math.abs(pinchZoom(200, 100) + 1) < 1e-9, 'חצי = -1')
  assert.equal(pinchZoom(100, 100), 0, 'בלי תזוזה — בלי זום')
  assert.equal(pinchZoom(0, 100), 0, 'ובלי מרחק התחלתי — לא מחלקים באפס')
})
...
test('שאיבה: שתיים מתוך חמש נושאות דמות, שלוש נותנות מטבעות', () => {
  ...
  assert.equal(coinsOnly.run.coinsTaken, WIND_COINS + SUCK_COINS)
  assert.equal(coinsOnly.run.lastWind.kind, 'suck')
  assert.equal(coinsOnly.run.lastWind.spat, null)
...
// האלומה לא יכלה להאיר עליו לעולם, והוא התגלה רק דרך רשת הביטחון.
test('האלומה: מי שעל הרצפה נמצא בתוכה, ומי שבצד לא', () => {
  const FOV = 62, BEAM_Y = 0.74, RX = 33, RY = 16
  const hit = (dx, dy, aspect) => {
    const bx = (dx / (FOV / 2)) * 50
    const by = ((52 - dy * 1.5) - BEAM_Y * 100) * aspect
    return (bx * bx) / (RX * RX) + (by * by) / (RY * RY) <= 1
  }
  const SHADOW_ELEV = -16          // ar/controllers/chase.js: ELEV.shadow.FAR
  for (const [w, h] of [[402, 880], [390, 844], [430, 932], [360, 800]]) {
    const a = h / w
    assert.ok(hit(0, SHADOW_ELEV, a), `מכוונים ישר אליו — האור עליו (${w}×${h})`)
    assert.ok(hit(10, SHADOW_ELEV, a), 'ועשר מעלות הצידה עדיין סלחני')
    assert.ok(!hit(22, SHADOW_ELEV, a), 'אבל עשרים ושתיים — כבר מחוץ לאלומה')
  }
  // ובגובה האופק, שם הכתם כבר לא נמצא
  assert.ok(!hit(0, 0, 880 / 402), 'מי שבגובה העיניים אינו בכתם שעל הרצפה')
})
