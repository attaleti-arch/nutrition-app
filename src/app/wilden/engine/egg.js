// ─── הביצה ───
// "יאללה ביצה זה קליל." קונים ביצה, היא נוסעת על הביקון, ההליכה מחממת
// אותה, ובפורטל היא בוקעת: יצור שכבר נתפס, בצבע נדיר. הילד לא יודע
// מראש מי ובאיזה צבע — זה מה שמחזיק.
//
// ── ולמה בדבש ──
// "מה בסוף עם הביצים, למה הם משמשים, אם כסף קונה חיות בצבעים?"
// כי היה כאן קיצור דרך: משרוקית הזהב נתנה צבע במטבעות, בלי ביצה ובלי
// הליכה, והחנות מכרה סקין מדויק — אז לביצה, שדורשת קילומטר וחצי של
// הליכה כדי לבקוע, לא נשאר שום יתרון. עכשיו שתיהן בדבש, והדבש בא
// מפרחים: אין דרך לצבע שלא עוברת ברגליים.
// ההבדל ביניהן נשאר מה שהוא היה: הביצה זולה יותר ומפתיעה (מי? איזה
// צבע?), והמשרוקית יקרה יותר ומיידית.
//
...
export const EGG_HONEY = 4        // הביצה בדבש: ארבע צנצנות
export const EGG_PRICE = EGG_HONEY   // שם ישן, לתאימות
...
export function canBuyEgg(progress) {
  return !progress.egg
    && (progress.res?.honey || 0) >= EGG_HONEY
