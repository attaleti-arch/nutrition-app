      ...
      {pieces.map(function(i) {
        ...
        return (
          <div key={i} style={{
            ...
            animation: 'fall ' + (2 + (i % 3) * 0.5) + 's ' + delay + 's ease-in forwards',
            opacity: 0.9,
          }} />
        )
      })}
    ...
    <div style={{ direction: 'rtl', marginBottom: 12 }}>
      <div style={{ background: 'linear-gradient(135deg,#0f4c2a,#16a34a)', borderRadius: 18, padding: '16px 18px', marginBottom: 10, color: '#fff' }}>
        ...
        <div style={{ fontSize: 13, color: '#bbf7d0', lineHeight: 1.6 }}>
          היי {clientName ? clientName.split(' ')[0] : ''}! המשוב האישי שלי עבורך 🌿
        </div>
      </div>
          ...
          {winLines.length > 0 && (
            <div style={{ background: '#f0fdf4', border: '1.5px solid #86efac', borderRadius: 16, padding: '14px 16px', marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                ...
                <span style={{ fontWeight: 800, fontSize: 14, color: '#166534' }}>מה עבד — שמרי על זה!</span>
              </div>
              {winLines.map(function(line, i) {
                return <div key={i} style={{ display: 'flex', gap: 8, padding: '6px 10px', background: '#dcfce7', borderRadius: 10, marginBottom: 6 }}>
          ...
          {focusLines.length > 0 && (
            <div style={{ background: '#fffbeb', border: '1.5px solid #fcd34d', borderRadius: 16, padding: '14px 16px', marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                <span style={{ fontSize: 18 }}>🎯</span>
                <span style={{ fontWeight: 800, fontSize: 14, color: '#92400e' }}>משימות לדיוק</span>
              </div>
              <div style={{ fontSize: 12, color: '#78350f', marginBottom: 8 }}>לא ביקורת — רק כיוון 💛</div>
              {focusLines.map(function(line, i) {
                return <div key={i} style={{ display: 'flex', gap: 8, padding: '6px 10px', background: '#fef3c7', borderRadius: 10, marginBottom: 6 }}>
                  <span style={{ fontSize: 13, color: '#92400e', flexShrink: 0 }}>→</span>
          ...
          {stepsLines.length > 0 && (
            <div style={{ background: '#eff6ff', border: '1.5px solid #93c5fd', borderRadius: 16, padding: '14px 16px', marginBottom: 10 }}>
              ...
              {stepsLines.map(function(line, i) {
                return <div key={i} style={{ display: 'flex', gap: 10, padding: '8px 10px', background: '#dbeafe', borderRadius: 10, marginBottom: 6 }}>
        ...
        <div style={{ background: '#fff', border: '1.5px solid #e5e7eb', borderRadius: 16, padding: '16px 18px', marginBottom: 10 }}>
          ...
          {feedback.length > 200 && (
            <button onClick={function() { setExpanded(!expanded) }} style={{ marginTop: 8, background: 'transparent', border: 'none', color: '#16a34a', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
              {expanded ? '▲ פחות' : '▼ קראי הכל'}
            </button>
          )}
        </div>
      )}
      {onOpenFull && (
        <button onClick={onOpenFull} style={{ width: '100%', padding: 11, borderRadius: 12, background: '#0f4c2a', color: '#fff', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 13 }}>
          📄 פתחי את המשוב המלא
        </button>
      )}
    </div>
...
export default function PlanApp({ clientName, userPassword }) {
  ...
  useEffect(function() {
    async function load() {
      ...
      if (todayLog.data) {
        var t = todayLog.data
        setChecks(t.checks || {})
        setCarbSel(t.carb_sel); setProtSel(t.prot_sel); setFatSel(t.fat_sel)
        ...
        if (t.nlp_metrics) {
          var m = t.nlp_metrics
          setStressLevel(m.stress || 0); setFatigueLevel(m.fatigue || 0)
          setHungerLevel(m.hunger || 0); setUserMood(m.mood || null)
        }
      }
    }
    if (dbKey) load()
  ...
  const saveProfile = async function() {
    ...
    await supabase.from('clients').update({
      ...
      sport_type: sportType, sport_commit_days: sportCommitDays,
    }).eq('password', dbKey)
    setProfileDone(true)
  }
  ...
  const handleSave = async function() {
    ...
    a.href = 'https://wa.me/' + trainerPhone + '?text=' + encodeURIComponent(notifyMsg)
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f0fdf4', padding: 24, direction: 'rtl' }}>
        ...
        <div style={{ width: '100%', maxWidth: 340, background: '#fff', borderRadius: 20, padding: 20, marginBottom: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
            {[
              { label: 'משקל (ק"ג)', val: userWeight, set: setUserWeight, ph: '70' },
              ...
              <div key={f.label}>
                ...
                <input type="number" value={f.val} onChange={e => f.set(e.target.value)} placeholder={f.ph} style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1.5px solid #e5e7eb', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} />
              </div>
            ))}
          </div>
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {['נקבה', 'זכר'].map(g => <button key={g} onClick={() => setUserGender(g)} style={{ flex: 1, padding: 10, borderRadius: 10, border: '2px solid ' + (userGender === g ? C.greenMid : '#e5e7eb'), background: userGender === g ? C.greenLight : '#fafafa', cursor: 'pointer', fontWeight: 700, fontSize: 14, color: userGender === g ? C.greenDark : '#555' }}>{g}</button>)}
            </div>
          </div>
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 12, color: '#555', marginBottom: 6, fontWeight: 600 }}>רמת פעילות</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {ACTIVITY_LEVELS.map(a => <button key={a} onClick={() => setUserActivity(a)} style={{ padding: '8px 12px', borderRadius: 99, border: '2px solid ' + (userActivity === a ? C.greenMid : '#e5e7eb'), background: userActivity === a ? C.greenLight : '#fafafa', cursor: 'pointer', fontSize: 13, fontWeight: 600, color: userActivity === a ? C.greenDark : '#555' }}>{a}</button>)}
            </div>
          </div>
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 12, color: '#555', marginBottom: 6, fontWeight: 600 }}>מטרה</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {GOALS_LIST.map(g => <button key={g} onClick={() => setUserGoal(g)} style={{ padding: '10px 14px', borderRadius: 10, border: '2px solid ' + (userGoal === g ? C.greenMid : '#e5e7eb'), background: userGoal === g ? C.greenLight : '#fafafa', cursor: 'pointer', fontSize: 14, fontWeight: 600, color: userGoal === g ? C.greenDark : '#555', textAlign: 'right' }}>{g}</button>)}
            </div>
          </div>
        </div>
        ...
        <button onClick={() => setProfileDone(true)} style={{ marginTop: 10, background: 'transparent', border: 'none', color: '#9ca3af', fontSize: 13, cursor: 'pointer' }}>
          {gf('דלגי', 'דלג')} בינתיים
        </button>
      </div>
    )
  ...
  if (showStage2Welcome) {
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg,#0f4c2a,#16a34a)', direction: 'rtl', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, color: '#fff' }}>
        ...
        {videoUrl && (
          <div style={{ width: '100%', maxWidth: 400, borderRadius: 16, overflow: 'hidden', marginBottom: 24, boxShadow: '0 8px 40px #0000004a' }}>
            <iframe src={videoUrl.replace('watch?v=', 'embed/')} width="100%" height="220" frameBorder="0" allowFullScreen style={{ display: 'block' }} />
          </div>
        )}
        <div style={{ background: '#ffffff20', borderRadius: 16, padding: 16, width: '100%', maxWidth: 400, marginBottom: 24 }}>
          ...
          {['🏃 ספורט נוסף על ההליכות — בחירה שלך', '🥫 מדריך מזווה ומקרר — מוכן לך כאן', '📸 צילום צלחת — AI יזהה מה אכלת', '🛒 מדריך קניות חכם — פתוח עבורך'].map((t, i) => (
            <div key={i} style={{ fontSize: 14, color: '#fff', padding: '6px 0', borderBottom: i < 3 ? '1px solid #ffffff15' : 'none' }}>{t}</div>
          ))}
        </div>
        <button onClick={() => setShowStage2Welcome(false)} style={{ padding: '16px 40px', borderRadius: 16, background: '#fff', color: '#0f4c2a', border: 'none', cursor: 'pointer', fontWeight: 900, fontSize: 17 }}>
          {gf('בואי', 'בוא')} נתחיל! 🚀
        </button>
      </div>
    )
    ...
    <div style={{ minHeight: '100vh', background: '#f8fafc', direction: 'rtl' }}>
      ...
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '10px 14px 0' }}>
        <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
          <button onClick={() => setActiveTab('diary')} style={{ flex: 1, padding: '10px 8px', borderRadius: 12, border: '2px solid ' + (activeTab === 'diary' ? '#16a34a' : '#e5e7eb'), background: activeTab === 'diary' ? '#dcfce7' : '#fff', cursor: 'pointer', fontWeight: 700, fontSize: 13, color: activeTab === 'diary' ? '#0f4c2a' : '#555' }}>
            📅 היומן
          </button>
          {currentStage >= 2 && (
            <button onClick={() => setActiveTab('guides')} style={{ flex: 1, padding: '10px 8px', borderRadius: 12, border: '2px solid ' + (activeTab === 'guides' ? C.teal : '#e5e7eb'), background: activeTab === 'guides' ? C.tealLight : '#fff', cursor: 'pointer', fontWeight: 700, fontSize: 13, color: activeTab === 'guides' ? C.teal : '#555' }}>
              🏡 מדריכים
            </button>
          )}
          {reportApproved && (
            <button onClick={() => setActiveTab('report')} style={{ flex: 1, padding: '10px 8px', borderRadius: 12, border: '2px solid ' + (activeTab === 'report' ? '#4a9b8e' : '#e5e7eb'), background: activeTab === 'report' ? '#e8f5f2' : '#fff', cursor: 'pointer', fontWeight: 700, fontSize: 13, color: activeTab === 'report' ? '#4a9b8e' : '#555' }}>
              🌿 הדוח
            </button>
          )}
        </div>
        ...
        <div style={{ maxWidth: 520, margin: '0 auto', padding: '0 14px 80px' }}>
          {videoUrl && (
            <div style={{ background: '#fff', borderRadius: 18, overflow: 'hidden', border: '1.5px solid #f0f0f0', marginBottom: 12 }}>
              <div style={{ padding: '14px 18px', background: 'linear-gradient(135deg,#0f4c2a,#16a34a)', color: '#fff', fontWeight: 800, fontSize: 15 }}>
                🎬 ברכת פתיחה מאתי
              </div>
          ...
          <div style={{ background: '#fff', borderRadius: 18, border: '1.5px solid #f0f0f0', marginBottom: 12, overflow: 'hidden' }}>
            <div style={{ padding: '14px 18px', background: C.tealLight, borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <div style={{ fontWeight: 800, fontSize: 15, color: C.teal }}>מדריך קניות חכם</div>
                <div style={{ fontSize: 12, color: '#9ca3af' }}>רשימה מלאה עם טיפים וצ׳קבוקסים</div>
              </div>
            </div>
          ...
          {/* מדריך מזווה — נפתח אחרי פגישה בבית */}
          {pantryNotes ? (
            <div style={{ background: '#fff', borderRadius: 18, border: '1.5px solid #f0f0f0', marginBottom: 12, overflow: 'hidden' }}>
              <div style={{ padding: '14px 18px', background: '#fff7ed', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: 22 }}>🏡</span>
                <div>
                  <div style={{ fontWeight: 800, fontSize: 15, color: C.orange }}>מדריך המזווה שלי</div>
                  <div style={{ fontSize: 12, color: '#9ca3af' }}>הנחיות אישיות מהפגישה שלנו בבית</div>
                </div>
              </div>
              <div style={{ padding: 14 }}>
      ...
      <div style={{ maxWidth: 520, margin: '0 auto', padding: '14px 14px 80px', display: activeTab === 'diary' ? 'block' : 'none' }}>
        ...
        {feedback && (
          <FeedbackCard feedback={feedback} clientName={displayName} logDate={today} onOpenFull={reportApproved ? () => setActiveTab('report') : undefined} />
        )}
            ...
            {!sportType ? (
              <>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
                  {SPORT_OPTIONS.map(s => (
                    <button key={s.key} onClick={async () => { setSportType(s.key); await supabase.from('clients').update({ sport_type: s.key }).eq('password', dbKey) }} style={{ padding: '8px 14px', borderRadius: 10, border: '1.5px solid #e5e7eb', background: '#fafafa', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>
                      {s.icon} {s.label}
                    </button>
                  ))}
                </div>
              ...
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  <div>
                    <span style={{ fontSize: 22 }}>{SPORT_OPTIONS.find(s => s.key === sportType)?.icon || '⚡'}</span>
                    <span style={{ fontWeight: 700, fontSize: 14, marginRight: 6, color: C.purple }}>{SPORT_OPTIONS.find(s => s.key === sportType)?.label || sportType}</span>
                  </div>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {[1, 2, 3, 4].map(n => (
                      <button key={n} onClick={async () => { setSportCommitDays(n); await supabase.from('clients').update({ sport_commit_days: n }).eq('password', dbKey) }} style={{ flex: 1, padding: '8px 0', borderRadius: 8, border: '2px solid ' + (sportCommitDays === n ? C.purple : '#e5e7eb'), background: sportCommitDays === n ? C.purpleLight : '#fff', color: sportCommitDays === n ? C.purple : '#555', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
                        {n}×
                      </button>
                    ))}
                  </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: sportDoneToday ? '#faf5ff' : '#fafafa', borderRadius: 12, padding: '12px 14px' }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: '#333' }}>אימון היום</div>
                    <div style={{ fontSize: 11, color: '#9ca3af' }}>השבוע: {sportDaysThisWeek}/{sportCommitDays} אימונים</div>
                  </div>
                  <button onClick={() => { var done = !sportDoneToday; setSportDoneToday(done); if (done) setSportDaysThisWeek(w => Math.min(w + 1, sportCommitDays)) }} style={{ padding: '8px 16px', borderRadius: 10, background: sportDoneToday ? C.purple : '#fff', color: sportDoneToday ? '#fff' : C.purple, border: '2px solid ' + C.purple, cursor: 'pointer', fontWeight: 700, fontSize: 13 }}>
                    {sportDoneToday ? '✅ בוצע!' : '+ סמן אימון'}
                  </button>
                </div>

                {sportDaysThisWeek >= sportCommitDays && (
                  <div style={{ marginTop: 8, background: C.purpleLight, borderRadius: 10, padding: '8px 12px', textAlign: 'center', fontSize: 13, color: C.purple, fontWeight: 700 }}>
                    🏆 {gf('השלמת', 'השלמת')} את המחויבות השבועית! מדהים!
                  </div>
                )}
              </>
            )}
        ...
        <Section title="ארוחת צהריים" icon="🌞" accent={C.greenMid} light={C.greenLight}>
          <div style={{ display: 'flex', gap: 8, padding: '10px 0' }}>
            {[{ k: 'A', l: '🍽️ מרכיבי הארוחה' }, { k: 'B', l: '🫒 רטבים ונלווים' }].map(opt => (
              <button key={opt.k} onClick={() => setLunchOpt(lunchOpt === opt.k ? null : opt.k)} style={{ flex: 1, padding: '10px 8px', borderRadius: 12, border: '2px solid ' + (lunchOpt === opt.k ? C.greenMid : '#e5e7eb'), background: lunchOpt === opt.k ? C.greenLight : '#fafafa', cursor: 'pointer', fontWeight: 700, fontSize: 12, color: lunchOpt === opt.k ? C.greenDark : '#555' }}>{opt.l}</button>
            ))}
          </div>
          {lunchOpt === 'A' && (
            <div>
              <div style={{ fontWeight: 700, fontSize: 12, color: C.greenMid, padding: '6px 0 2px', textAlign: 'right' }}>פחמימה:</div>
              {filteredCarbs.map(o => <RadioRow key={o.id} id={o.id} text={o.text} accent={C.greenMid} selected={carbSel} onSelect={setCarbSel} />)}
              <div style={{ fontWeight: 700, fontSize: 12, color: C.greenMid, padding: '10px 0 2px', textAlign: 'right' }}>חלבון:</div>
              {filteredProt.map(o => <RadioRow key={o.id} id={o.id} text={o.text} accent={C.greenMid} selected={protSel} onSelect={setProtSel} />)}
            </div>
          )}
          {lunchOpt === 'B' && (
            <div>
              <div style={{ fontWeight: 700, fontSize: 12, color: C.greenMid, padding: '6px 0 2px', textAlign: 'right' }}>רטבים ותוספות:</div>
              {filteredFat.map(o => <CheckRow key={o.id} id={o.id} text={o.text} accent={C.greenMid} checked={!!checks[o.id]} onToggle={id => setChecks(c => { var n = {...c}; n[id] = !n[id]; return n })} />)}
            </div>
          )}
        ...
        <Section title="ארוחת ערב" icon="🌙" accent={C.purple} light={C.purpleLight}>
          ...
          {PLAN.veggieOptions.map(o => (
            <div key={o.id + '_e'} onClick={() => setChecks(c => { var n = {...c}; n[o.id + '_erev'] = !n[o.id + '_erev']; return n })} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: '1px solid #f3f4f6', cursor: 'pointer', opacity: checks[o.id + '_erev'] ? 0.45 : 1 }}>
        ...
        <Section title="🥫 המזווה ועוגני ההתארגנות" icon="🛒" accent={C.teal} light={C.tealLight} isLocked={currentStage < 2} lockMessage="ייפתח לאחר פגישת המזווה שלנו! 🔒">
          <div style={{ paddingTop: 6 }}>
            ...
            <div style={{ fontSize: 13, color: '#444', background: C.tealLight, padding: 10, borderRadius: 10, lineHeight: 1.6, marginBottom: 12 }}>
              {gf('הקפידי', 'הקפד')} שבכל סופ"ש יהיו לך לפחות 3 מקורות חלבון מבושלים במקרר וירקות שטופים וחתוכים.
            </div>
            {[
              { id: 'shop1', text: 'הכנתי קיטים מראש למקפיא' },
              { id: 'shop2', text: 'חתכתי ירקות ושמתי בגובה העיניים במקרר' },
              { id: 'shop3', text: 'יש קופסת הצלה מוכנה במקרר' },
              { id: 'shop4', text: 'קניתי מקורות חלבון לשבוע' },
            ].map(item => <CheckRow key={item.id} id={item.id} text={item.text} accent={C.teal} checked={!!checks[item.id]} onToggle={id => setChecks(c => { var n = {...c}; n[id] = !n[id]; return n })} />)}
          </div>
        </Section>
          ...
          <div style={{ padding: '10px 0' }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {Array.from({ length: 8 }).map((_, i) => (
                <button key={i} onClick={() => setWater(i < water ? i : i + 1)} style={{ width: 38, height: 38, borderRadius: 10, fontSize: 18, cursor: 'pointer', border: '2px solid ' + (i < water ? C.blue : '#e5e7eb'), background: i < water ? C.blueLight : '#fafafa' }}>💧</button>
              ))}
            </div>
          </div>
        </Section>
        ...
        <button onClick={() => setSetupDone(false)} style={{ width: '100%', padding: 10, borderRadius: 12, marginTop: 6, background: 'transparent', border: '1px solid #e5e7eb', color: '#9ca3af', cursor: 'pointer', fontSize: 13 }}>
          {gf('עדכני', 'עדכן')} העדפות תזונה
        </button>
        <button onClick={() => setProfileDone(false)} style={{ width: '100%', padding: 10, borderRadius: 12, marginTop: 6, background: 'transparent', border: '1px solid #e5e7eb', color: '#9ca3af', cursor: 'pointer', fontSize: 13 }}>
