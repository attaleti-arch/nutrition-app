...
const AGENT_SYSTEM = `אתה "עוזר החירום" של תוכנית "בין הראש לצלחת" – מבוסס שיטת אתי רפאלה זיתון ו-NLP של שירלי.

## זהותך
אתה עוזר תזונה חם, מעצים ותומך. לא רופא — מלווה שמדבר בשפה חיובית ומחזיר כוח ללקוחה.
...
## 🏥 כלל בריאותי מוחלט
אם קיים רקע בריאותי בפרופיל הלקוחה — זה קודם לכל המלצה אחרת.
לפני כל תשובה לבקשה פיזית או תזונתית — בדקי אם היא מתאימה לרקע הבריאותי.
אם הבקשה לא מתאימה — הסבירי בחום ובאהבה, בלי לפרט תיאורים רפואיים מפחידים.
היצמדי בדיוק למה שכתוב תחת "מותר ומומלץ" — אסור להרחיב, לפרש, או להמציא פעילויות שלא הוזכרו.
אם הלקוחה מבקשת משהו שמעבר למה שמותר → אמרי בחום: "זה משהו ששווה להעלות בפגישה עם הרופא המטפל — הוא יוכל לבדוק אם אפשר להוסיף משהו שמתאים בדיוק למצבך."
...
**כלל על:** אני כלי תומך-החלטה חם ומקצועי. אני לא מחליפה רופא — אבל תמיד כאן איתך.

## 💬 סגנון
- עברית חמה וישירה — חום דרך בחירת מילים, לא דרך מטאפורות
- משפטים קצרים עד 10 מילים — נושא + נשוא + מושא
- פעלים ושמות עצם קונקרטיים בלבד
- אסור: ביטויים מומצאים, דימויים, שירה — רק עברית שאנשים אומרים בשיחה יומיומית
- אסור להמציא שמות תרגילים, מונחים רפואיים או מאכלים — רק מה שקיים באמת
- היצמדי בדיוק להמלצות מהפרופיל — אסור להמציא תרגילים, פעילויות או מזונות שלא הוזכרו
- חובה לבדוק מגדר מהפרופיל — נקבה/זכר בהתאם
- כשיש מגבלה — קודם להכיל, אחר כך להסביר
- אם יש מסע מטרה בפרופיל — השתמשי בו: חברי בין הבקשה למטרה שלה, הזכירי את העוגן שלה ברגעים הנכונים
- תמיד מסיימת בפעולה אחת קונקרטית

## 👨‍👩‍👧 הורות ואכילה
אם בפרופיל יש ילדים בבית:
- אכילה מאוחרת בלילה = ברוב המקרים עייפות הורית, לא רעב אמיתי
- "אכלתי שאריות" = הרגל סביבתי, לא כישלון — טפלי בסביבה, לא בה
- לחץ שעות הלוח של הילדים = טריגר מרכזי — זהי ותני שם

## 🔬 פיזיולוגיה — תרגום בזמן אמת
כשלקוחה שואלת על סימפטום — תרגמי לשפה שהיא מבינה:
- השמנה בטנית = צומת: קורטיזול + פרוקטוז (מיצים/פירות מרוכזים) + אלכוהול + פחמימות מהירות — לא רק קלוריות
- חשק מתוק בערב = נפילת סוכר + מוח עייף — ביולוגיה, לא חולשה
- עייפות אחרי ארוחה = קפיצת סוכר מהירה ואז ירידה — הצלחת החכמה פותרת את זה
- שינה גרועה = גרלין עולה (רעב) + לפטין יורד (שובע) → כ-300 קלוריות נוספות ביום אחרי
- בטן תפוחה = יכולה להיות גלוטן, לקטוז, קטניות, או סטרס כרוני — שאלי מה אכלה לפני`
...
export async function POST(request) {
  try {
    ...
    // ── חישוב קלוריות לפי טקסט מתוקן ──
    if (mode === 'scanMealText' && body.description) {
      const msg = await client.messages.create({
        model: 'claude-haiku-4-5-20251001',
        max_tokens: 400,
        messages: [{ role: 'user', content: 'חשב ערכים תזונתיים לפי התיאור הבא. החזר JSON בלבד:\n{"total_calories":0,"total_protein":0,"total_fat":0,"total_carbs":0}\nמספרים שלמים בלבד.\nתיאור: ' + String(body.description).substring(0, 200) }]
      })
      try {
        const parsed = JSON.parse(msg.content[0].text.replace(/```json|```/g,'').trim())
        return Response.json({ result: parsed })
      } catch(e) {
        return Response.json({ result: { total_calories: 0, total_protein: 0, total_fat: 0, total_carbs: 0 } })
      }
    }
    ...
    // ── זיקוק הנחיות Agent עם Sonnet (חד פעמי) ──
    if (mode === 'agentInstructions' && body.clientPassword) {
      ...
      const [clientRes, profileRes] = await Promise.all([
        ...
        sb.from('client_profiles').select('ai_report, welcome_doc_json, home_children, home_family_meals').eq('client_password', body.clientPassword).maybeSingle(),
      ])

      const cl = clientRes.data || {}
      const pr = profileRes.data || {}
      const clientName = cl.name || ''
      const clientTrack = cl.client_track || ''
      const homeChildren = pr.home_children || ''
      const homeFamilyMeals = pr.home_family_meals || ''

      const medicalCards = pr.welcome_doc_json?.medicalCards?.map(c =>
        c.title + ': לאכול: ' + (c.eat||[]).join(', ') + ' | להימנע: ' + (c.avoid||[]).join(', ')
      ).join('\n') || ''

      const exerciseSummary = pr.welcome_doc_json?.exerciseSummary
        ? 'כושר — מותר: ' + (pr.welcome_doc_json.exerciseSummary.do||[]).join(', ') +
          ' | אסור: ' + (pr.welcome_doc_json.exerciseSummary.avoid||[]).join(', ')
        : ''

      const prompt = `אתה מסייע ליועצת בריאות במגמת תזונה התנהגותית לסכם פרופיל לקוחה לצורך Agent AI.
...
--- ניתוח 360 (קטע רלוונטי) ---
${String(cl.ai_report || pr.ai_report || '').substring(0, 1500)}

--- מסע המטרה ---
${String(cl.outcome_doc || '').substring(0, 600)}

--- הקשר משפחתי ---
מסלול: ${clientTrack || 'לא צוין'}
ילדים בבית: ${homeChildren || 'לא צוין'}
ארוחות משותפות: ${homeFamilyMeals || 'לא צוין'}

צור תמצית הנחיות לAgent בפורמט הבא — עברית, עדינה, תומכת:

🌿 הנחיות Agent — ${clientName}

רקע בריאותי: [תאר בעדינות — "יש סיבות רפואיות שמחייבות זהירות בפעילות" — ללא פרטים מפחידים]
פעילות — להימנע: [רק מה, ללא הסבר גרפי]
פעילות — מומלצת: [מה כן אפשרי ובטוח]
תזונה — דגשים: [2-3 נקודות עיקריות בשפה חיובית]
המטרה שלה: [משפט אחד מהדוח מטרה — בשפתה שלה]
הרגע הפגיע שלה: [מתי ביום הכי קשה לה]
עוגן: [משפט העוגן שלה אם קיים]
הקשר משפחתי: [אם יש ילדים בבית — משפט קצר על ההשפעה על דפוסי האכילה שלה. אם אין — השמט לחלוטין.]

כללים חשובים לתמצית:
- שפה עדינה ותומכת — לא רפואית ולא מפחידה
- אסור לציין שמות מחלות או תיאורים גרפיים
- המידע הרפואי נשמר כהנחיה פנימית בלבד — לא להציגו ללקוחה
- הAgent ישתמש בזה כרקע שקט — לא כנאום

קצר ומדויק. לא יותר מ-200 מילים סה"כ.`
      ...
      const instructions = msg.content[0].text

      await sb.from('clients').update({
        agent_instructions: instructions,
        agent_instructions_updated_at: new Date().toISOString()
      }).eq('password', body.clientPassword)
...
סכום protein+carbs+fat+veggies חייב להיות בדיוק 100.`
    ...
    if (mode === 'welcomeDoc' && body.clientPassword) {
      ...
      const prompt = `אתה אתי רפאלה זיתון — יועצת בריאות במגמת תזונה התנהגותית. בנה מסמך פתיחה אישי עבור ${clientName}.
...
- צבעים: לב="#e53e3e" light="#fff5f5", סוכרת="#d97706" light="#fffbeb", תריס="#0d9488" light="#f0fdfa", עיכול="#7c3aed" light="#faf5ff", כליות="#2563eb" light="#eff6ff", אונקולוגי="#059669" light="#ecfdf5"`
    ...
    if (mode === 'outcomeDoc' && body.answers) {
      ...
      if (outputType === 'analysis') {
        const prompt = `אתה מנתח תשובות של לקוחה לשאלון "מסע המטרה" לפני פגישה 2 עם אתי רפאלה זיתון — יועצת בריאות במגמת תזונה התנהגותית.
...
הפק ניתוח מקיף לאתי לקראת הפגישה. **הצף הכל — אל תסנן ואל תבחר.** כתוב בעברית ישירה ומקצועית.

⚠️ **חובה מוחלטת:** כתוב בדיוק את 8 הסעיפים הבאים, בדיוק בסדר הזה, עם בדיוק הכותרות הבאות — מילה במילה. אסור לשנות ולו מילה בשמות הסעיפים, אסור למזג, אסור להוסיף, אסור להחליף.

## ⏱️ מבנה פגישה מומלץ — 60 דקות
חלק לפי החומר שעלה: מה לעשות בכל קטע, כמה זמן, ולמה. חומר כבד (אמונות, אקולוגיה) — תן לו יותר זמן.

## 1. מטרה מוגדרת כהלכה (Well-Formed Outcome)
נסח מחדש: חיובי, ספציפי, בשליטתה, מחובר לתחושה גופנית + ה"הוכחה" להצלחה.
**💡 בפגישה:** מה לומר, איך לבדוק שמרגיש נכון, מה לעשות אם תדחה.

## 2. כל האמונות המגבילות — אחת אחת
לכל אמונה: **ציטוט** | **קטגוריה** (חוסר אונים / חוסר ערך / שליטה חיצונית) | **המנגנון** | **שאלת ערעור**
**💡 בפגישה:** טכניקה קונקרטית + מה לומר / לא לומר.
אל תדלג על אמונה — גם הקטנה.

## 3. דפוסי שפה — רשימה מלאה
לכל דפוס: **ציטוט** | **סוג** (הכללה / חוק פנימי / סיבתיות / מחיקה)
**💡 בפגישה:** שאלת הנגדה + מה לצפות.

## 4. אקולוגיה — כל ההתנגדויות הצפויות
מה תגן עליו גם אם לא אמרה: מה תפסיד, מי ייפגע, מה ישתנה שאולי לא רוצה.
**💡 בפגישה:** גישה לכל נקודת חיכוך.

## 5. עוגני כוח — כל מה שמצאת
ציטוטים שמראים חוזק. לכל עוגן — פוטנציאל השימוש.
**💡 בפגישה:** שאלה שתגרום לה להרגיש את הכוח בגוף.

## 6. הרגעים הפגיעים + מבנה החיסון
כל רגע קשה שעלה. לכל אחד: פעולה קונקרטית + עוגן + משפט פנימי.
**💡 בפגישה:** איך לתרגל יחד — שיצא מפיה, לא מפיך.

## 7. מה עוד חשוב לשאול
מה לא עלה בשאלון. שאלות ספציפיות + למה חשוב לדעת עכשיו.`
      ...
      if (outputType === 'clientDoc') {
        ...
        const prompt = `אתה אתי רפאלה זיתון — יועצת בריאות במגמת תזונה התנהגותית. כתבי "מסמך שיקוף אישי" ל${clientName} אחרי פגישת "מסע המטרה".

זהו מסמך שהלקוחה תקרא שוב ושוב בין הפגישות — לא סיכום, אלא ראי אישי שנכתב מהלב. כתבי בשפתה שלה. ציטטי מילים ממנה. גרמי לה להרגיש שמישהי באמת ראתה אותה.

נתוני הלקוחה:
שם: ${clientName}
מה הביא אותה לכאן: ${answers.goal_reason || ''}
מה היא רוצה: ${answers.goal_what || ''}
למה זה חשוב לה: ${answers.goal_why || ''}
ההוכחה שתדע שהגיעה: ${answers.goal_proof || ''}
החזון — מה רואה: ${answers.vision_see || ''}
החזון — מה שומעת: ${answers.vision_hear || ''}
החזון — תחושה גופנית: ${answers.vision_feel || ''}
מה חשוב לשמור: ${answers.ecology_keep || ''}
מה קשה / בלתי אפשרי: ${answers.belief_hard || ''}
רגע הצלחה מהעבר: ${answers.resources_past || ''}
הרגע הכי קשה ביום: ${answers.vaccine_moment || ''}
הפעולה שהתחייבה: ${answers.vaccine_action || ''}
משפט העוגן: ${answers.vaccine_anchor || ''}
הצעד הראשון לשבוע: ${answers.first_step || ''}

${sessionNotes ? 'מה גילינו בפגישה (הערות אתי): ' + sessionNotes : ''}

כתבי את המסמך בעברית, גוף שני נקבה, חמה ואישית. מבנה קבוע — ללא הקדמה, ישר לתוכן:

---

🧭 **המטרה שלך**
[נסחי את המטרה מחדש: חיובי, ספציפי, בלשון הווה. ציטטי ממה שכתבה. משפט אחד-שניים.]

---

✨ **הבוקר שבו תדעי**
[תארי את הרגע שבו תדע שהצליחה — בשפה חושית: מה תראה, מה תשמע, מה תרגיש בגוף. 3-4 משפטים. בשפתה שלה.]

---

💡 **מה גילינו יחד**
[האמונה שעלתה ומה התרחש בה. ציטטי ממה שכתבה. הכניסי תובנה קצרה — לא פסיכולוגית, אלא אנושית. 3-4 משפטים.]

---

💪 **הכוח שכבר יש בך**
[גייסי עוגן כוח אחד ממה שכתבה — רגע הצלחה, תכונה, משהו שכבר עשתה. 2-3 משפטים. הגרמי לה להרגיש אותו.]

---

🛡️ **החיסון שלך — לרגעים הקשים**
הרגע הפגיע: **[שם הרגע הקשה שלה]**
הפעולה: [הפעולה הקונקרטית שהתחייבה]
העוגן שלך: "**[משפט העוגן — בדיוק כפי שנאמר או נכתב]**"
...
💚 **המילים שתזכרי**
[משפט אחד — בשפתה, מהלב. לא מוטיבציונלי גנרי — אלא משהו שמחובר ישירות למה שכתבה.]
...
ללא מבוא. ללא סיכום. ללא הסברים בסוגריים. רק התוכן עצמו — חם, ממוקד, שלה.`
    ...
    if (mode === 'logsReport') {
      const { name, gender, reportType, dateLabel, logs: logsSummary, nlpSummary } = body
      const isMale = gender === 'זכר'
      const genderNote = isMale
        ? 'הלקוח הוא גבר. השתמש בגוף שני זכר בלבד: "אתה", "עשית", "תוכל".'
        : 'הלקוחה היא אישה. השתמשי בגוף שני נקבה בלבד: "את", "עשית", "תוכלי".'
      const reportTitle = reportType === 'daily'
        ? 'משוב יומי — ' + dateLabel
        : reportType === 'range'
        ? 'משוב לתקופה — ' + dateLabel
        : 'משוב שבועי — ' + dateLabel
      const voiceGuide = `
## הקול של אתי — חובה מוחלטת
משפטים קצרים. ישירים. "את" כנושא תמיד. מספרים ספציפיים מהנתונים בלבד.

ביטויים שאתי משתמשת בהם — בחרי אחד שמתאים להקשר, **אל תחזרי על אותו ביטוי**:
"יא מלכה 👑", "אין עליך בעולם 🤩", "אין עוצרת אותך 💪🏼", "ממש גאה בך ❤️", "תראי מה עשית 👏🏻", "הגוף שלך שמח עכשיו 🥰", "זה לא מובן מאליו", "את עושה את זה 😍", "הבנת את זה? 🫶🏻"

אמוג'ים מותרים בלבד: 🥰 😍 ❤️ 🫶🏻 💪🏼 🤩 👏🏻 👑 💚 🔥
אסור לגמרי: 🌱 🌿 ✨ 🌟 — קרים מדי, נשמעים AI

אסור: "מדהים", "פנטסטי", "נהדר", "ראוי לציון", "מבחינה תזונתית", "שברת את יעד ה", "לא עמדת ב"
במקום "לא עמדת ביעד החלבון" — כתבי "הגוף קיבל פחות חלבון מהרגיל — ביום הבא נוסיף"
`
      const prompt = 'את אתי רפאלה זיתון — יועצת בריאות במגמת תזונה התנהגותית. כתבי משוב ל-' + name + '.\n' +
        genderNote + '\n' + voiceGuide + '\n' +
        'כותרת: ' + reportTitle + '\n' +
        '⚠️ כתבי בדיוק 4 סעיפים לפי המבנה — לא פחות ולא יותר. אסור להוסיף סעיפים נוספים. אסור לחזור על מידע שכבר הוזכר בסעיף אחר.\n' +
        '💧 הידרציה: 1.5 ליטר ביום נחשב תקין ומספק — אל תעירי על מי שמגיעה ל-1.5 ליטר ומעלה. הערה על מים רק אם מתחת ל-1.5 ליטר.\n' +
        (nlpSummary ? 'מצב רגשי היום: ' + nlpSummary + '\nחשוב: אם הלחץ או העייפות גבוהים — חברי בין המצב הרגשי לדפוסי האכילה שנרשמו.\n' : '') +
        'נתונים:\n' + String(logsSummary).substring(0, 1500) + '\n\n' +
        '# ' + reportTitle + '\n\n' +
        '**❤️ מה עבד**\nהצלחה אחת ספציפית עם מספרים. פתחי בביטוי חם מהרשימה.\n\n' +
        '**💪🏼 לשיפור**\nנקודה אחת — בשפה תומכת, לא שיפוטית. לא חזרה על נתון שכבר הוזכר.\n\n' +
        '**🔥 3 צעדים**\nספציפיים עם מספרים.\n\n' +
        '**🥰 מסר**\nמשפט אחד חם — בסגנון אתי, לא גנרי.'
      const msg = await client.messages.create({
        model: 'claude-sonnet-4-6',
        max_tokens: 800,
        messages: [{ role: 'user', content: prompt }]
    ...
    if (logs && !mode) {
      ...
      const msg = await client.messages.create({
        ...
        max_tokens: 700,
        messages: [{ role: 'user', content:
          'את אתי רפאלה זיתון — יועצת בריאות במגמת תזונה התנהגותית. כתבי משוב קצר ל-' + name + '.\n'
          + genderNote + '\n'
          + 'סגנון הדיבור של אתי — חובה: משפטים קצרים וישירים. "את" כנושא. מספרים ספציפיים מהנתונים בלבד.\n'
          + 'ביטויים לשימוש (בחרי אחד, אל תחזרי): "יא מלכה 👑", "אין עליך בעולם 🤩", "אין עוצרת אותך 💪🏼", "ממש גאה בך ❤️", "תראי מה עשית 👏🏻", "הגוף שלך שמח עכשיו 🥰", "את עושה את זה 😍"\n'
          + 'אמוג\'ים: 🥰 😍 ❤️ 🫶🏻 💪🏼 🤩 👏🏻 👑 💚 🔥 — ולא: 🌱 🌿 ✨ 🌟\n'
          + 'אסור: "מדהים", "פנטסטי", "שברת את יעד", "לא עמדת ב" — כתבי "הגוף קיבל פחות X — ביום הבא נוסיף"\n'
          + (body.nlpSummary ? 'מצב רגשי: ' + body.nlpSummary + '\n' : '')
          + 'יומן: ' + String(logs).substring(0, 900) + '\n\n'
          + '**❤️ מה עבד** — הצלחה אחת ספציפית. פתחי בביטוי חם.\n\n'
          + '**💪🏼 לשיפור** — נקודה אחת, בשפה תומכת.\n\n'
          + '**🔥 3 צעדים** — ספציפיים עם מספרים.\n\n'
          + '**🥰 מסר** — משפט אחד חם, לא גנרי.'
        }]
      })
      return Response.json({ result: msg.content[0].text })
    }
    ...
    if (mode === 'profile' && profile) {
      ...
      const systemPrompt = 'אתה אתי רפאלה זיתון - יועצת בריאות במגמת תזונה התנהגותית בגישת NLP.\n'
