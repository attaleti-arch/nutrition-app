...
export function buildGraph(ways, blocked = []) {
  ...
  for (const w of ways) {
    ...
    for (let i = 1; i < w.nodes.length; i++) {
      ...
      adj[ia].push({ to: ib, d, w: d * cost, tier: w.tier, back: f, name: w.name || null, kind: w.kind || null })
      adj[ib].push({ to: ia, d, w: d * cost, tier: w.tier, back: e, name: w.name || null, kind: w.kind || null })
    }
  }
  return { nodes, adj }
}
...
export function loopSteps(graph, loop) {
  return loop.map((idx, k) => {
    ...
    let street = null, kind = null
    if (k > 0) {
      const e = graph.adj[loop[k - 1]].find(x => x.to === idx)
      street = e?.name || null
      kind = e?.kind || null
    }
    // צומת: שלוש דרכים ומעלה נפגשות כאן. שם אסור להעמיד ילד — שם
    // מכוניות פונות. (ראה engine/placement.js, safeAlong.)
    const junction = (graph.adj[idx]?.length || 0) >= 3
    return { lat: n.lat, lng: n.lng, street, kind, junction }
  })
}
