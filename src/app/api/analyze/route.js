...
מה לא עלה בשאלון. שאלות ספציפיות + למה חשוב לדעת עכשיו.`
      ...
      if (outputType === 'clientDoc') {
        ...
        const prompt = `אתה אתי אטל — יועצת בריאות ותזונה התנהגותית. כתבי "מסמך שיקוף אישי" ל${clientName} אחרי פגישת "מסע המטרה".
...
`
      const prompt = 'את אתי אטל — יועצת בריאות במגמת תזונה התנהגותית. כתבי משוב ל-' + name + '.\n' +
        genderNote + '\n' + voiceGuide + '\n' +
        'כותרת: ' + reportTitle + '\n' +
    ...
    if (logs && !mode) {
      ...
      const msg = await client.messages.create({
        ...
        max_tokens: 700,
        messages: [{ role: 'user', content:
          'את אתי אטל — יועצת בריאות במגמת תזונה התנהגותית. כתבי משוב קצר ל-' + name + '.\n'
          + genderNote + '\n'
