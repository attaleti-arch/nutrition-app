...
export const CREATURES = {
  ...
  // מגוסטבו, אחרי שנים בפנים; ונקי — מי שהוא כשהוא בבית. הראשון מופיע
  // פעם אחת בחיי המשחק (ui/WhisperFreed), השני הוא הדמות.
  //
  // הקליפים שלה: live הוא שתי השניות וחצי הראשונות של הקליפ שבהן הוא
  // עומד ומזיז אוזניים; ההמשך — שבו הוא הולך אל המצלמה — הוא קליפ
  // התפיסה, כי אצלו התפיסה *היא* ההליכה אליכם (עומדים כמו פסל, והוא
  // בא). ולכן הוא גם לא מוצג כפתיח: לראות אותו הולך אליכם עוד לפני
  // שעמדתם הורס בדיוק את מה שהמפגש שלו מבקש.
  // ו-/world/whisper/freed.mp4 הוא הרגע שבו הרוח פולטת אותו.
  whisper: {
    id: 'whisper', name: 'ויספר',
    model: null,
    ios: null,
    clip: '/creatures/whisper/caught.mp4',
    introClip: false,
  ...
  // "הדרקון יהיה שושו". שם רך על דרקון — וגם מה שהוא עשה בתוך הרוח.
  drake: {
    id: 'drake', name: 'שושו',
    model: null,
    ios: null,
...
// ── הקליפ שפותח את המפגש ──
// לרוב היצורים זה אותו קליפ של התפיסה: רואים אותו רגע, ואז מחפשים.
// לוויספר לא: הקליפ שלו *הוא* התפיסה (הוא הולך אליכם), ולהראות אותו
// מראש מספר את הסוף. introClip:false אומר "יש קליפ, אבל לא בהתחלה".
export const revealClip = c => (c?.introClip === false ? null : c?.clip || null)
